...
> 2026-09-19 이후 실행 순서의 기준은 [MS 개발 에이전트 인계](../ms-v1/ms-agent-handoff.md)다. R0는 사용자 완료 판정이며 기존 M 단계 순차 진행은 중단한다. 아래 내용은 기존 구조/구현 이력 참고이며 MS 미구현 작업의 완료 근거가 아니다.

M0, R0 규칙형 기준선과 M1/M2 기반 코드·에셋·도구는 구현되어 있다. 아래 기존 구현 구조를 재사용하되 최신 실제 상태는 `docs/soccer/current-status.md`에서 확인한다.
...
| Runtime | MNG_RuleBasedManager.cs / MNG_R0RuleMatchMonitor.cs: 비학습 규칙형 감독 v0와 읽기 전용 경기 진단 |
| Runtime | MNG_HumanInput.cs / MNG_ControlOwnership.cs / MNG_HumanSurrogate.cs: 사람·대역·제어권 |
| Runtime | MNG_RewardEngine.cs / MNG_EventLedger.cs / MNG_TacticalRewardTracker.cs: 실제 기술 결과를 감독 보상 사건으로 변환·중복 방지 |
...
단계별 Scene은 Curriculum/M0_Technical/Scenes/MNG_M0_Technical.unity처럼 배치한다. 비학습 기준선은 `Curriculum/R0_RuleBaseline/Scenes/MNG_R0_RuleVsRule.unity`, 학습 단계는 M1_AttackChoice, M2_DefenseChoice, M3_FallbackMatch, M4_HumanSupport, M5_SelfPlay, M6_Tactics 형태다. 모든 Scene은 동일한 환경 Prefab과 실행 계약을 사용한다.
...
| RuleBasedManager | 학습 없이 snapshot에서6명령을 고르는 독립 감독. Agent/BehaviorParameters/DecisionRequester 금지, 선수 직접 이동·킥 금지 |
...
### R0 규칙형 감독 기준선 — M0 승인 뒤, M1 재개 전

- Red/Navy에 동일한 `MNG_RuleBasedManager` v0를 배치하고 기존 Fallback·PPO Agent·Human 입력을 모두 비활성화한다.
- 감독은0.5초마다 공 소유, 위치, 압박, 득점 차, 남은 시간을 읽어 `AdvanceCarry/PassBuild/AttemptShot/ActiveRecover/Balanced/ProtectBack` 중 하나를 결정한다. 대상 좌표와 선수별 수행은 PPO와 동일한 TacticalTargetResolver→MatchController→TeamPlanner→SkillExecutor 계층을 사용한다.
- 공격은 열린 슛, 압박 시 패스, 공간 전진 순으로 판단하고 수비는 문전 보호, 회수 가능 압박, 균형 복귀 순으로 판단한다. 최소1초 명령 유지로 과도한 전환을 막는다.
- 완료: 양 팀 규칙 감독만 활성, 신경망 컴포넌트0,6개 명령 단위상황 선택·좌우대칭,300초 경기 완주·유한 물리·명령/소유전환 확인. 자동 gate와 실제 대전의 패스·슛 발현을 확인하면 개발자가 임시 승인할 수 있고, 전용 Scene의 사람 육안 승인은 후속 최종 gate로 남긴다.
- R0 성적은 강화학습 성과가 아니다. 이후 R1부터 같은 물리·명령·Planner 계약을 유지한 채 규칙 우선순위와 공간 판단을 단계적으로 강화한다.

#### R0 후속 경기 보조 완료 — 2026-09-19

- 수비 필요 상황에는 서로 다른 위치와 책임을 갖는 최소 두 명을 배정한다. 1차 압박자는 운반자와 공을 직접 압박하고, 2차 압박자는 패스·돌파 경로를 차단해 한 명만 싸우고 나머지가 관전하는 상태를 금지한다.
- 공이 자기 골대에 가까울 때 상대 진영에 남은 미드필더는 공격 임무보다 골문 수비 복귀를 우선한다. 위협이 해소된 뒤에만 원래 포메이션 역할로 돌아간다.
- 전역 공 정체 시 자기 골대 방향이 아닌 안전한 킥 각도가 있을 때는 실제 킥 플레이트 슈팅·클리어를 첫 복구 단계로 시도한다. 자기 골대 방향이거나 자책골 위험 각도이면 슈팅을 금지하고 기존 방향 재탐지·다중 선수 위치 변경으로 바로 넘어간다. 안전 슈팅이 실패해도 기존 복구 절차를 이어간다.
- 세 기능은 규칙형 전용 휴리스틱이 아니라 `MatchController→TeamPlanner→PlayerSkillExecutor` 공통 실행 계약으로 구현하여 규칙형과 학습형에 동일하게 적용한다. 감독의 6개 명령·133-float 관측 계약은 유지한다.
- 완료: Planner v9의 단위 계약에서 기존 2인 압박·미드필더 복귀·정체 안전 슈팅에 더해, 상대 골대30m 구역에서 운반자 외 지원자1명이 공 전방·안쪽 측면으로 합류하는 최소2인 공격을 Red/Navy 양쪽에 확인했다. 패스·슛 힘1000 증가분을 출구속도 `+6.6667m/s`로 반영하고 공 최고속도·관측 정규화도36.6667m/s로 동기화했다. 실제 슛10/20m와 패스5/10/20m는 각20/20, MNG EditMode `75/75`, 전체 MNG PlayMode `25/25`, 독립 R0 3×300초, Builder `MNG M0 VALIDATION PASS`를 통과했다. 사용자 육안 승인은 계속 남아 있다.
