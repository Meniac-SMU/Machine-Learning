...
> 2026-09-19 이후 감독 RL 실행 인계는 [MS 개발 지침](../ms-v1/ms-agent-handoff.md), 판정은 [MS 평가 계약](../ms-v1/ms-validation.md)이 우선한다. 아래 기존 도구와 명령은 기존 M 단계용이며 MS 옵션/병렬 실행을 이미 지원한다고 가정하지 않는다. R0는 사용자 완료 판정이다.

작업 루트 C:/GitHub/Machine-Learning. M0 런타임, 비학습 R0 규칙형 대전, M1 공격 선택, M2 수비 선택의 Player·훈련·frozen ONNX 독립 평가 경로를 구현하고 실제 실행했다. M3 학습과 frozen 300초×40경기 평가 경로도 구현했으나 정식 평가는 아직 실행 전이다. 범용 inspect/manifest 통합 도구는 아직 미구현이다.
...
R0 자동 검증은 규칙 감독2개만 활성, 신경망 컴포넌트0,6명령 단위상황·좌우대칭,300초 완주·finite 물리·명령/소유전환을 확인한다. Planner v9부터 기존 2인 수비 압박·미드필더 복귀·정체 안전 슈팅과 함께, 자기 소유 공이 상대 골대30m 구역에 있을 때 운반자와 별도 지원자 최소2명이 공격에 참가하는지를 확인한다. 2026-09-19 자동 회귀는 통과했지만 사용자가 직접 `MNG_R0_Visual_Approval_Guide.md`로 경기 체감을 승인하기 전에는 R0 최종 합격으로 표시하지 않는다.
...
- 정책 집합: 무작위 유효명령, 초기 미학습, MNG_Fallback, `MNG_RuleBasedManager` R0/R1..., M3 Base, 최종Base, Attack/Defense/Press, self-play 과거snapshot.
- 모든 정책은 같은6명령·Planner v9/executor·공질량3.0·크기1.1·마찰0.05·반발0.15·공 최고속도36.6667m/s·safety를 사용한다. 규칙형/PPO 모두 2인 수비 압박, 상대 골문 근처 2인 공격 가담, 미드필더 골문 복귀, 안전한 정체 슈팅 우선, 골키퍼 배급과 증가된 패스·슛 세기를 같은 공통 코드에서 사용한다.
...
- checkpoint 평가는 승률/득점/실점뿐 아니라6명령 선택수, 명령별 mask 가능률, 실제 플레이트 Pass/Shot, 패스수신,5m전진을 기록한다. 두 연속 평가에서 주요성능3%p 이상 증가가 없고 행동·사건 다양성도 개선되지 않으면 max_steps 전이라도 Ctrl+C 정상저장 후 원인을 분석한다.
