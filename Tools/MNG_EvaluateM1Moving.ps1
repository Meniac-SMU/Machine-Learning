...
if ($RunId -notmatch '^MNG_M1Moving-\d{8}-r\d{3}$') {
    throw "Run ID must match MNG_M1Moving-YYYYMMDD-rNNN. Received: $RunId"
}
if ([string]::IsNullOrWhiteSpace($EvidenceId)) {
    $EvidenceId = "$RunId-eval-r001"
}
if ($EvidenceId -notmatch '^MNG_M1Moving-\d{8}-r\d{3}-eval-r\d{3}$') {
    throw "Evidence ID must match MNG_M1Moving-YYYYMMDD-rNNN-eval-rNNN. Received: $EvidenceId"
}
...
$buildDirectory = Join-Path $projectRoot "Builds\MNG_M1MovingEvaluation\$RunId"
$executablePath = Join-Path $buildDirectory 'MNG_M1MovingEvaluation.exe'
$levelDataPath = Join-Path $buildDirectory 'MNG_M1MovingEvaluation_Data\level0'
...
if (Get-Process Unity -ErrorAction SilentlyContinue) {
    throw 'Close the Unity Editor before starting the MNG M1 moving evaluation build.'
}
...
if ($unityProcess.ExitCode -ne 0) {
    throw "MNG M1 moving evaluation build failed with exit code $($unityProcess.ExitCode). See $buildLogPath"
}

foreach ($requiredPath in @($executablePath, $levelDataPath, $buildInfoPath)) {
    if (-not (Test-Path -LiteralPath $requiredPath -PathType Leaf)) {
        throw "MNG M1 moving evaluation build artifact is missing: $requiredPath"
    }
}
...
if ($sourceHash -ne [string]$buildInfo.modelSha256) {
    throw 'Frozen M1 moving ONNX hash does not match the evaluation build manifest.'
}
if ((Get-Sha256Lower $executablePath) -ne [string]$buildInfo.executableSha256) {
    throw 'M1 moving evaluation executable hash does not match its build manifest.'
}
if ((Get-Sha256Lower $levelDataPath) -ne [string]$buildInfo.levelDataSha256) {
    throw 'M1 moving evaluation level-data hash does not match its build manifest.'
}
...
if ($playerProcess.ExitCode -ne 0) {
    throw "MNG M1 moving evaluation Player failed with exit code $($playerProcess.ExitCode). See $playerLogPath"
}
if (-not (Test-Path -LiteralPath $resultPath -PathType Leaf)) {
    throw "MNG M1 moving evaluation did not create a result: $resultPath"
}

$result = Get-Content -Raw -Encoding UTF8 -LiteralPath $resultPath | ConvertFrom-Json
if ([int]$result.scenarios -ne 100) {
    throw "MNG M1 moving evaluation did not finish exactly 100 scenarios: $($result.scenarios)"
}
if ([string]$result.runId -ne $RunId -or [string]$result.modelSha256 -ne $sourceHash) {
    throw 'MNG M1 moving evaluation result identity does not match the frozen candidate.'
}
if (-not [bool]$result.movingNavyDefense) {
    throw 'MNG M1 moving evaluation did not report moving Navy defense.'
}
...
[PSCustomObject]@{
    ...
    MovingNavyDefense = [bool]$result.movingNavyDefense
