...
if ([string]::IsNullOrWhiteSpace($OutputDirectory)) {
    $OutputDirectory = Join-Path $projectRoot 'Logs\MNG-MS\MS3-source-20260921'
}
if (Test-Path -LiteralPath $OutputDirectory) {
    throw "MS3 snapshot directory already exists; preserve it and choose another path: $OutputDirectory"
}
...
if ($LASTEXITCODE -ne 0) { throw 'Could not capture the MS3 untracked list.' }

$initialization = Join-Path $projectRoot 'results\MNG_MS2-20260921-r005\MNG_Manager\MNG_Manager-99968.pt'
$manifest = [ordered]@{
    schema = 'MNG-MS3-source-v1'
    capturedUtc = [DateTime]::UtcNow.ToString('O')
    gitHead = $head
    snapshotRoot = 'source-snapshot'
    fileCount = $manifestFiles.Count
    initialization = [ordered]@{
        candidate = 'MNG_MS2-20260921-r005-step99968'
        status = 'provisional-pass'
        path = 'results/MNG_MS2-20260921-r005/MNG_Manager/MNG_Manager-99968.pt'
        sha256 = Get-Sha256Lower $initialization
    }
    files = $manifestFiles
}
$manifest | ConvertTo-Json -Depth 7 | Set-Content -Encoding UTF8 -LiteralPath (Join-Path $OutputDirectory 'source-sha256.json')
Write-Host "MNG MS3 SOURCE SNAPSHOT PASS files=$($manifestFiles.Count) output=$OutputDirectory"
