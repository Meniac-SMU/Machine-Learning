...
상태: M0 구현·자동 검증 완료, 사람의 실제 화면/입력 검토 대기. 변경 시 schema/Profile/테스트/문서를 동기화한다.
...
- 위치는 Geometry 반길이/반폭으로 나누고 골문 내부를 위해 [-1.2,1.2] clamp. 선수속도/9, 공속도/30. heading=평면 단위벡터.
...
추격자는 예상도착시간 최소, tie=고정slot.0.5초 배정 유지 후 재검토, Human 제외. Keeper는 기존 Home/Engage 원칙을 MNG 순수 함수로 포팅한다. MNG Profile에 정확한 기존 수치를 옮기고 변경 이력 기록.
...
- pass범위5~28m, 약킥14m/s, 강킥28m/s, 공최고30m/s. 자동 패스는 실측 damping을 반영해10m 이하는14m/s, 10~20m는14→28m/s 선형 보간, 20m 이상은28m/s를 사용한다. 초기 강킥24m/s는 실제 공 damping에서 열린 골문20m에 도달하지 못해 M0 물리 gate 근거로28m/s로 확정했다.
- pass 후보: 미래 수신위치 선형예측+경로차단+전진성+동료 압박 평가, 동률slot. 실제 맞힐 수 없는 후보는 제외. Human도 수신대상 가능하나 이동 강제 금지.
...
- 현재 우선순위: 위험지역 중앙clearance→유효24m근거리슛→7m압박 때5~28m패스→전진10m/lane3m운반.
